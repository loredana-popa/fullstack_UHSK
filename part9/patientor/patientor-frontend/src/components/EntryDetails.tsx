import { Entry, OcupationalHealthcare, HospitalEntry, HealthCheckEntry, healthCheckRating} from "../types"
...
const HospitalEntryDetails = (props: HospitalEntryDetailsProps) => {
  return (
  <div>
    <div>{props.date} <LocalHospital/> </div>
    <div><em>{props.description}</em></div>
    <ul>
        {props.diagnosisCodes?.map((c) => 
          <li key={c}>{c}</li>
        )}
      </ul>
...
const OccupationalHealthcareEntryDetails = (props:OccupationalHealthcareProps) => {
  return(
    <div>
      <div>{props.date} <Work/> {props.employerName}</div>
      <div><em>{props.description}</em></div>
      <ul>
        {props.diagnosisCodes?.map((c) => 
          <li key={c}>{c}</li>
...
const HealthCheckEntryDetails = (props:HealthCheckEntryProps)=>{
  return(
    <div>
      <div>{props.date} <HealthAndSafety/> </div>
      <div><em>{props.description}</em></div>
